...
var (
	dhSinglePassStdDhSha1kdf = asnECDHAlgorithm{
		Algorithm: doScheme(x963Scheme, []int{2}),
	}
	dhSinglePassStdDhSha256kdf = asnECDHAlgorithm{
		Algorithm: doScheme(secgScheme, []int{11, 1}),
	}
	dhSinglePassStdDhSha384kdf = asnECDHAlgorithm{
		Algorithm: doScheme(secgScheme, []int{11, 2}),
	}
	dhSinglePassStdDhSha224kdf = asnECDHAlgorithm{
		Algorithm: doScheme(secgScheme, []int{11, 0}),
	}
	dhSinglePassStdDhSha512kdf = asnECDHAlgorithm{
...
func UnmarshalPrivate(in []byte) (prv *PrivateKey, err error) {
	...
	pub, err := UnmarshalPublic(ecprv.Public.Bytes)
	if err != nil {
		return nil, err
	}
	prv.PublicKey = *pub
